---
title: "TaxoFlow's output report and analysis"
author: "User"
date: "2026"
output:
    rmdformats::downcute:
...
<p style='text-align: justify;'>We used Kraken2/Bracken to perform taxonomic assignment of the raw sequences and estimate species abundance. The procedure included the remotion of reads mapping to the [*Arabidopsis thaliana* genome](https://benlangmead.github.io/aws-indexes/bowtie). The workflow is proposed by [Lu et al. (2022)](https://www.nature.com/articles/s41596-022-00738-y), wrapped with Nextflow.</p>

<p style='text-align: justify;'>The analysis of the raw counts was developed using the species abundance (Bracken output) data as a Phyloseq object to display absolute and relative abundance, α and β-Diversity.</p>
...
ps_genus_counts <- tax_glom(data_biom, taxrank = "Genus")
ps_genus_rel <- transform_sample_counts(
  ps_genus_counts,
  function(x) x / sum(x)
)

genus_mean_rel <- taxa_sums(ps_genus_rel) / nsamples(ps_genus_rel)
keep_genera <- genus_mean_rel >= 0.03
data_biom <- prune_taxa(keep_genera, ps_genus_counts)
...
sample_data(data_biom)$SampleID <- sample_names(data_biom)
data.ord <- ordinate(data_biom, "PCoA", "bray")
plot_ordination(data_biom, data.ord, type="samples",
                title="Samples", color = "SampleID")+
    theme_bw()+
    ...
    labs(title="Samples", color = "Samples")
```
