...
4. **Taxonomic classification** with [**Kraken2**](https://ccb.jhu.edu/software/kraken2/). This tool relies on an indexed database that can be [downloaded](https://benlangmead.github.io/aws-indexes/k2). Here, we will use the custom database with 54 bacterial species that we have selected only for the purpose of this tutorial. However, you can expand the annotation simply by switching to another database.
