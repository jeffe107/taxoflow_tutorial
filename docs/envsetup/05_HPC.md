...
<p style='text-align: justify;'>If you are aiming at installing and executing the pipeline on an HPC cluster, you must be aware that probably you can't rely on Docker as it requires _root_ privileges. Fortunately, Nextflow can easily handle this for you by just changing the container images and enabling Apptainer/Singularity in the config files. For instance:</p>

On a given process this would be a Singularity/Apptainer container created using Seqera Containers for an AMD64 architecture:
